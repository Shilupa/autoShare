...
const getHomeCars = async () => {
  try {
    const queries =
      "SELECT c.reg_no, c.brand, c.model, r.rating, r.comment, c.seater, c.fuel_type, c.transmission, c.rent_price, homePics.file_name, p.name as carOwner, bp.name as bookingPerson " +
      "FROM car c " +
      ...
      "left outer join (SELECT * FROM pictures group by car_reg_no) homePics on c.reg_no = homePics.car_reg_no";
